...
router.get('/clientOrders', verifyLogin, async function (req, res, next) {
  let orders = await productHelpers.getOrderPage()
  let admin = req.session.admin
  res.render('admin/clientOrders', { admin, orders });
});

router.get('/viewClientOrders/:id', verifyLogin, async (req, res) => {
  let orderItems = await userHelpers.getViewOrderProducts(req.params.id)
  let admin = req.session.admin
  res.render('admin/viewClientOrders', { orderItems, admin })
